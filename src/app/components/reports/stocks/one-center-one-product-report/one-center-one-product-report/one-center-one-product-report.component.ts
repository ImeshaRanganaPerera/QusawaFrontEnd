...
@Component({
  selector: 'app-one-center-one-product-report',
  standalone: true,
  imports: [MaterialModule],
  templateUrl: './one-center-one-product-report.component.html',
  styleUrl: './one-center-one-product-report.component.scss'
})
export class OneCenterOneProductReportComponent implements OnInit {
  isSpinning = true;
  reportDate!: string;
  centerId!: string;
  productId!: string;
  ...
  // loadStockData() {
  //   this.productService.getStockByCenterAndProduct(this.centerId, this.productId).subscribe((res: APIResponse) => {
  //     const data = res.data;
